...
sub Run {
    ...
    for my $Key (qw( Subject Body PriorityID TypeID IPAddress ServiceID SLAID Expand Dest FromChatID)) {
    ...
    if ( !$Self->{Subaction} ) {
		...
		#Raja Edit Capture of IP
		my $DBObject=$Kernel::OM->Get('Kernel::System::DB');
		if ($TicketID)
		{
		my $FieldID = '20';
        $Kernel::OM->Get('Kernel::System::DB')->Do(
		    SQL => 'INSERT INTO dynamic_field_value (field_id,object_id, value_text) VALUES (?, ?, ? )' ,       
            Bind  => [ \$FieldID,\$TicketID, \$GetParam{IPAddress},  ],        
        );
		# $Kernel::OM->Get('Kernel::System::DynamicFieldValue')->_DeleteFromCache(
            # FieldID => $FieldID,
            # ObjectID => $TicketID,
        # );
