...
sub Run {
    ...
    if ($HasContent) {
        ...
        for my $StateOrderID ( sort { $a <=> $b } keys %ConfiguredStates ) {
			...
			#Raja
			$TotalUrl.="$QueueIDURL;StateIDs=$States{ $ConfiguredStates{$StateOrderID} };";			
			$TotalCount = $TotalCount + $StatusTotal[$StateOrderID];
		...
		$LayoutObject->Block(
				Name => 'ContentLargeTicketTotal',
				Data => {
					Number   => $TotalCount,
					QueueIDs  => $TotalUrl,
					#StateIDs => $TotalStateUrl,
					#Sort     => $Sort,
				},
			);
    }
